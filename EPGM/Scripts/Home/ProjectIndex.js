...
jQuery(document).ready(function () {
    ...
    $(".subBut").click(function () {
        ...
        window.location.href = urlContent + "Sector/Index/?dW5kZWZpbmVk=" + whoType + "&BeneType=" + $("#BeneType option:selected").val() + "&Month=" + $("#ddlMonth option:selected").val() + "&Year=" + $("#txtYear").val();
    });
...
function BindGrid() {
    ...
    grid.jqGrid({
        ...
        colNames: ['DistrictID', 'S.No', 'District Code', 'District Name', 'Project Code', 'Project Name', 'Sector Code', 'Sector Name', 'Total Beneficiaries', 'No.of Children Weighed', 'Attendance Count', 'Normal', 'Moderate', 'Severe', 'Normal', 'MAM', 'SAM', 'Normal', 'Moderate', 'Severe', 'Absent'],
        colModel: [
                ...
                { name: 'SectorCode', index: 'SectorCode', width: 160, hidden: true },
		  		{ name: 'SectorName', index: 'SectorName', width: 140, formatter: showDistricts, sortable: true },
                { name: 'BeneCount', index: 'BeneCount', width: 120, align: 'center' },
...
function showDistricts(cellvalue, options, rowObject) {
    return "<a href='" + urlContent + "Home/Report/?distCode=" + rowObject.DistrictCode + "&projCode=" + rowObject.ProjectCode + "&seccode=" + rowObject.SectorCode + "'style='text-decoration: underline;color:red'>" + rowObject.SectorName + "</a>";
}
