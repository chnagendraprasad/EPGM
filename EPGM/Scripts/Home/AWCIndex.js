...
jQuery(document).ready(function () {
    ...
    $(".subBut").click(function () {
        ...
        window.location.href = urlContent + "District/Index/?dW5kZWZpbmVk=" + whoType + "&Month=" + $("#ddlMonth option:selected").val() + "&Year=" + $("#txtYear").val();
    });
...
function BindGrid() {
    ...
    grid.jqGrid({
        ...
        beforeRequest: function () {
            ...
            $.extend(postData, { "StateCode": $("#StateID option:selected").val(), "BeneType": $("#BeneType option:selected").val(), "Month": $("#ddlMonth option:selected").val(), "Year": $("#txtYear").val() });
        },
        colNames: ['DistrictID', 'S.No', 'District Code', 'District Name', 'Project Code', 'Project Name', 'Sector Code', 'Sector Name', 'Center Code', 'AWC Name', 'Total Beneficiaries', 'No.of Children Weighed', 'Attendance Count', 'Normal', 'Moderate', 'Severe', 'Normal', 'MAM', 'SAM', 'Normal', 'Moderate', 'Severe', 'Absent'],
        colModel: [
                ...
                { name: 'Row', index: 'Row', width: 50, align: 'center', sortable: false, hidden: true },
		  		...
		  		{ name: 'DistrictName', index: 'DistrictName', width: 160, formatter: showDistricts, sortable: true, hidden: true },
                { name: 'ProjectCode', index: 'ProjectCode', width: 160, hidden: true },
		  		{ name: 'ProjectName', index: 'ProjectName', width: 160, formatter: showDistricts, sortable: true, hidden: true },
                { name: 'SectorCode', index: 'SectorCode', width: 160, hidden: true },
		  		{ name: 'SectorName', index: 'SectorName', width: 100, formatter: showDistricts, sortable: true, hidden: true },
                { name: 'AWCCode', index: 'AWCCode', width: 160, hidden: true },
		  		{ name: 'AWCName', index: 'AWCName', width: 140, formatter: showDistricts, sortable: true },
		  		{ name: 'BeneCount', index: 'BeneCount', width: 130, align: 'center' },
                ...
                { name: 'TotalAttendance', index: 'TotalAttendance', width: 120, align: 'center', sortable: true, sorttype: 'number' },
		  		{ name: 'Normal', index: 'Normal', width: 110, align: 'center', sortable: true, sorttype: 'number' },
...
function BindData() {
    ...
    $.post("/Home/GetDashStats?BeneType=" + $("#BeneType option:selected").val() + "&Month=" + $("#ddlMonth option:selected").val() + "&Year=" + $("#txtYear").val(), "", function (result) {
...
function showDistricts(cellvalue, options, rowObject) {
    ...
    return "<a href='" + urlContent + "Home/Report/?distCode=" + rowObject.DistrictCode + "&projCode=" + rowObject.ProjectCode + "&seccode=" + rowObject.SectorCode + "&awccode=" + rowObject.AWCCode + "'style='text-decoration: underline;color:red'>" + rowObject.AWCName + "</a>";
}
function Checkyear() {
    var date = new Date();
    if ($('#ddlMonth').val() == "0" || $('#ddlMonth').val() == "" || $('#ddlMonth').val() == null) {
